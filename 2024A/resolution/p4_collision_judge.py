"""
p4_collision_judge.py
分析一组数据点，以判断在特定的运动轨迹中是否存在潜在的碰撞。代码中定义了两个函数.
"""
...
def judge_if_collision(ids, it_x, it_y):
    ...
    potential_queue = ids[closest_id - 2: closest_id + 3]  # 获取潜在碰撞点的ID集合

    h0_id, h1_id = ids[0], ids[1]
    xh0, yh0 = it_x[h0_id], it_y[h0_id]
    xh1, yh1 = it_x[h1_id], it_y[h1_id]
    head_hypotenuse_length = cal_distance(3.41 / 2, 0, medium_point_center_distance(xh0, xh1, yh0, yh1) + 0.15)
...
plt.plot(limit_iter_x, limit_iter_y)
plot_by_ids(res_idx, False, limit_iter_x, limit_iter_y)

# 获取最有可能导致碰撞的点的索引
closest_id = get_most_potential_id_for_collision(res_idx, limit_iter_x, limit_iter_y)
print(closest_id)

# 获取潜在碰撞点的ID集合
potential_queue = res_idx[closest_id - 1: closest_id + 2]
plt.plot(limit_iter_x[potential_queue], limit_iter_y[potential_queue])

# 显示图形
plt.show()

# 判断是否发生碰撞
judge_if_collision(res_idx, limit_iter_x, limit_iter_y)